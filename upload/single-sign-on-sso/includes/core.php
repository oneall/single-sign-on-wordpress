    ...
    if (oa_single_sign_on_is_configured())
    {
        ...
        switch (strtolower($status->action))
        {
            ...
            case 'new_user_no_login_autocreate_off':
                // Grace Period
                oa_single_sign_on_set_login_wait_cookie($ext_settings['blocked_wait_relogin']);

                // Add Log.
                oa_single_sign_on_add_log('[INIT] @' . $status->action . '] Guest detected but account creation is disabled. Blocking automatic SSO re-login for [' . $ext_settings['blocked_wait_relogin'] . '] seconds, until [' . date("d/m/y H:i:s", $ext_settings['blocked_wait_relogin']) . ']');
            ...
            case 'existing_user_no_login_autolink_off_unverified_emails':
                ...
                // Add log.
                oa_single_sign_on_add_log('[INIT] @' . $status->action . '] - Blocking automatic SSO re-login for [' . $ext_settings['blocked_wait_relogin'] . '] seconds, until [' . date("d/m/y H:i:s", $ext_settings['blocked_wait_relogin']) . ']');
            ...
            default:
                ...
                if (is_user_logged_in())
                {
                    ...
                    else
                    {
                        ...
                        $oa_single_sign_on_user_token = get_user_meta($user->ID, 'oa_single_sign_on_user_token', true);

                        if (empty($oa_single_sign_on_identity_token) || empty($oa_single_sign_on_user_token))
                        {
                            // Update user to cloud storage.
                            $status_cloud_user = oa_single_sign_on_core_synchronize_user_to_cloud_storage($user);

                            // Save Cloud User
                            if ($status_cloud_user->is_successfull === true)
                            {
                                // Update All token to be up-to-date with OneAll Database
                                oa_single_sign_on_add_local_storage_tokens_for_user($user, $status_cloud_user->user_token, $status_cloud_user->identity_token);

                                $oa_single_sign_on_identity_token = $status_cloud_user->identity_token;
                            }
                        }

                        // Update SSO Token
                        $synchronized_sso_token = oa_single_sign_on_core_get_session_for_identity_token($oa_single_sign_on_identity_token);
                        ...
                        if (!empty($synchronized_sso_token) && $synchronized_sso_token->is_successfull === true)
                        {
                            ...
                            oa_single_sign_on_add_local_sso_session_token_for_user($user, $synchronized_sso_token->sso_session_token);

                            // No distant SSO token => remove local one
                        }
                        else
                        {
                            oa_single_sign_on_remove_local_sso_session_token_for_user($user);
                        }
                    }
                }
                else
